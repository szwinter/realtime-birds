...
def fit_migration_model(y_train, lat_train, day_train, threshold_train, theta_mean, theta_prec,
                        theta_init = None, lr=0.1, num_epochs=10000, tol=0.0001):
    y_train_tensor = torch.tensor(y_train, dtype=torch.float32)
    lat_train_tensor = torch.tensor(lat_train, dtype=torch.float32)
    day_train_tensor = torch.tensor(day_train, dtype=torch.float32)
    threshold_train_tensor = torch.tensor(threshold_train, dtype=torch.float32)
    theta_mean_tensor = torch.tensor(theta_mean, dtype=torch.float32)
    
    grid_vec = torch.cartesian_prod(torch.arange(60.0,70.0, 1), torch.arange(1.0,366.0, 10))
    
    if theta_init is None:
        theta = torch.tensor(theta_mean_tensor.detach().numpy(), requires_grad=True)
    else:
        theta = torch.tensor(theta_init, requires_grad=True, dtype=torch.float32)
        
    optimizer = torch.optim.Adam([theta], lr=lr)
    losses = []
    for epoch in tqdm.tqdm(range(num_epochs), desc="Training migration model"):
        optimizer.zero_grad()
